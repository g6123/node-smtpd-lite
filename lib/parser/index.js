		...
		switch(this._status) {
			...
			case 1:
				...
				if(transition) {
					...
					line = this._parseBody(line, this.header['content-transfer-encoding']);
			...
			case 3:
				...
				if(transition) {
					...
					line = this._parseBody(line, this._currentPart['header']['content-transfer-encoding']);
...
Parser.prototype._parseHeader = function(line) {
	...
	var i18nDecoder = new SkippingDecoder(/=\?[A-Za-z0-9-]+\?[BbQq]\?[^\?]+\?=/g, mimeDecoders.decodeMimeWord);
...
Parser.prototype._parseBody = function(line, encoding) {

	if(encoding) {
 	   
		var encoding = encoding.toLowerCase();
		...
		if(encoding == 'base64') {
			...
			if(line[line.length-1] === '=') {
				...
				this._lastQuote = line.substr(0, line.length-1);
				line = '';

			} else {

				this._lastQuote = '';
				line = mimeDecoders.decodeQuotedString(line)+'\n';
